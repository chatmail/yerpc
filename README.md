...
## Example
```bash
cargo new my-example && cd my-example
cargo add yerpc --features support-axum
cargo add tokio --features rt-multi-thread
cargo add axum serde_json typescript_type_def
```

**src/main.rs**
```rust
use axum::Extension;
use axum::{Router, extract::ws::WebSocketUpgrade, response::Response, routing::get};
use std::net::SocketAddr;
use yerpc::axum::handle_ws_rpc;
use yerpc::{RpcClient, RpcSession, rpc};
...
After running `cargo build` you will find an autogenerated TypeScript client in the `typescript/generated` folder and an `openrpc.json` file in the root of your project.
